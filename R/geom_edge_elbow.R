...
#' @export
geom_edge_elbow <- function(mapping = NULL, data = getEdges, stat = "edge_elbow",
...
StatEdgeElbow2 <- ggproto('StatEdgeElbow2', Stat,
    compute_panel = function(data, scales, flipped = TRUE, n = 100) {
        posCols <- c('x', 'y', 'group', 'circular', 'direction')
...
#' @importFrom ggplot2 layer GeomPath aes
#' @export
geom_edge_elbow2 <- function(mapping = NULL, data = getEdges, stat = "edge_elbow2",
                            position = "identity", arrow = NULL,
                            lineend = "butt", na.rm = FALSE, show.legend = NA,
                            inherit.aes = TRUE, n = 100, ...) {
    mapping <- aesIntersect(mapping, aes(x=x, y=y, circular=circular, direction=direction))
    layer(data = data, mapping = mapping, stat = stat, geom = GeomPath,
          position = position, show.legend = show.legend, inherit.aes = inherit.aes,
          params = list(arrow = arrow, lineend = lineend, na.rm = na.rm, n = n, ...)
    )
