...
#' when calling \code{\link{ggraph}}. If the graph is either a tree or a forest
...
layout_tbl_graph_auto <- function(graph, circular, ...) {
    if (with_graph(graph, graph_is_tree() || graph_is_forest())) {
...
add_direction <- function(graph, pos, direction = 'out') {
    ...
    graph <- mutate(graph, direction = ifelse(pos$x[.data$to] < pos$x[.data$from], 'left', 'right'))
    if (direction == 'in') {
